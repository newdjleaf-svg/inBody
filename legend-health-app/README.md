# v13 更新
- N1～N5 營養換算依最新照片基準。
- 六大營養素新增食物文字快速輸入、常用食物快捷鍵與即時估算。
- 儲存時將食物估算自動加入每日目標，仍保留手動營養素修正。
- 新增蛋白質、碳水、好油脂、纖維、蔬果支持份量提示。

# v12 更新
- 新增睡眠時間紀錄：入睡、起床、睡眠品質與紀錄時間。
- 自動計算跨午夜睡眠時數，每日理想值 7～8 小時。
- 3 天與 14 天模式皆可使用，並同步到完整報告與 A4 匯出。
- 不加入健康設備同步。

# v11 更新
- 以使用者最後確認的「NP產品建議喝法－每日營養整合表」為主要營養換算基準。
- 蛋白質目標改為體重×1.6÷7；碳水目標改為體重×3÷15。
- N1/N2/N3 自動計纖維；N4 計蛋白質；N5 計好油脂；N1/N2/N3/N4/N5 計碳水。
- N1/N2/N4/S1 加入維生素支持，N3 加入礦物質支持；支持份只做APP達成度，不代表實際微量營養素含量。
- 新增依客戶身體、血糖、血壓、血脂、用藥、營養紀錄自動產生的飲食指南。
- 新增蛋白質、全穀雜糧、纖維與好油脂的食物份量快速參考。

# v10 產品營養換算版

新增 N1/N2/N3/N4/N5/超氧/超抗/S1 分次用量、沖泡水量併入飲水、已知纖維/蛋白質/碳水自動換算，以及產品簡易介紹選單。

# v9 更新
- 各板塊以 A4 直式輸出可讀性重新調整，放大輸出字體並盡量避免卡片跨頁切割。
- 新增運動目標：每日 6,000 步。
- 步行可分次輸入步數、持續時間、時間戳，儲存後自動累計並清空欄位。
- 依目前體重與步數估算步行熱量消耗，報告同步顯示。
...
## v7 新增
- 六大營養素：水、蛋白質、膳食纖維、好油脂、維生素與礦物質、碳水化合物。
- 3 天速成 / 14 天習慣養成切換。
- 飲水量可手動輸入 cc，水杯水位依每日目標比例同步。
- 保留 v5 血糖、血壓、血脂、用藥、Web Push、PDF/PNG 匯出。
